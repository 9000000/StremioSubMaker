...
- **Token Vault profile manager on the config page:** Saved session tokens now behave like real browser-local profiles instead of loose strings you have to keep copying around. A new floating Vault launcher opens a quick-switch rail and full profile manager where you can keep up to 5 profiles, rename them, jump between them, and switch away from dirty drafts with explicit save/discard prompts instead of losing work. Each profile can be enabled or disabled, copy its manifest/config URL, install directly in Stremio, open its matching History or Sub Toolbox page, validate itself, and be imported/exported through raw tokens, manifest/config URLs, single-profile JSON, or full-vault backups. Live, draft, recovered, and disabled states are surfaced directly in the UI, and when the vault is full SubMaker previews the oldest local profile that will be replaced before anything is purged.

**Improvements:**

- **Refreshed bundled anime ID mappings:** Updated `data/anime-list-full.json` with newer upstream IMDb, TMDB, TVDB, and season mappings for anime entries. `animeIdResolver` now also understands the newer structured `season` shape in the upstream data and skips known placeholder target-ID pairs while loading the bundled file, reducing the chance of bad rows poisoning offline anime ID resolution.

**Bug Fixes:**

- **Fixed the config-page "Reset settings to default" flow still behaving like a pre-Token-Vault-era wipe hint instead of a real modern local reset:** The old reset CTA and modal copy were written before the config page gained Token Vault / Profile Manager, Quick Setup draft persistence, What's New state, per-tool/browser-local preferences, and the newer split between local browser state and server-side session tokens. That left the warning incomplete and misleading, and the flow itself reloaded into a freshly minted saved token instead of a true blank local first-run state. The config-page reset now performs an explicit browser-local purge of SubMaker site data for that origin: local/session storage, cached config/token references, Token Vault entries, Quick Setup progress, dismissed help/preferences, cookies, service worker registrations, Cache Storage, and any IndexedDB created there. It then reloads `/configure` as a fresh blank draft instead of auto-selecting a newly regenerated saved profile. The reset CTA/modal copy was also rewritten and localized to clearly spell out what is cleared, what returns to defaults, and what is intentionally **not** deleted: server-side tokens/profiles remain intact and can still be reopened later if the user still has the token URL elsewhere.

- **Fixed the config page instructions modal "Don't show this again" preference not reliably sticking:** The config page now uses one canonical suppression key, migrates `hideConfigInstructions` values forward automatically, persists both checking and unchecking the box instead of only writing on close, and keeps the checkbox UI in sync when reopening the help modal manually. Also removed the leftover help-modal script that was double-binding the same popup with its own storage key, and stopped treating the dismissal preference like disposable version/cache UI state so app updates or cache migrations no longer silently re-enable the popup.

- **Fixed ASS/SSA archives from fansub packs degrading to `WEBVTT undefined` or empty output after archive extraction:** Some `.ass`/`.ssa` files inside subtitle archives include blank spacer lines inside the `[Events]` section (for example between `Format:` and the first `Dialogue:` line). `subsrt-ts` treats those blank lines as section breaks, so it would parse only ASS metadata/styles, emit a meta-only VTT containing a literal `undefined`, and our enhanced ASS converter would correctly reject the result while the archive fallback path still accepted it because it only checked for a non-empty string. `preprocessASS()` now strips blank lines inside `[Events]`, `postprocessVTT()` now normalizes line endings and removes bogus numeric cue identifiers inherited from skipped meta/style rows, and the archive ASS fallback now post-processes plus validates `subsrt-ts` VTT output before accepting it, falling through to safer recovery paths when the library still misbehaves. Also tightened VTT validation so cue-id-only output no longer counts as real subtitle text, and hardened ASS/VTT→SRT conversion to validate plus renumber successful SRT output so translation paths cannot keep malformed or meta-shifted cue numbering from third-party converters.

- **Fixed SubDL episode searches keeping explicit multi-episode packs that do not contain the requested episode:** SubDL can return multi-episode packs with real `episode_from`/`episode_end` metadata ranges such as episodes 5-7 even when the user is searching for episode 2. We already marked those results as season packs, but the episode filter kept all season packs unconditionally, which meant users could still be shown a pack that was guaranteed to fail later with "episode not found in this subtitle pack." `searchSubtitles()` in `subdl.js` now keeps full-season packs, but filters out explicit multi-episode packs whose range does not include the requested episode.

- **Fixed Gemini translation model selection disappearing on the config page after reloads, version migrations, and Quick Setup handoffs:** The main Gemini model dropdown intentionally exposes only the curated static model list, while other config paths still used backend/default aliases such as `gemini-flash-latest` or request-time advanced override models. When the config page hydrated the base `<select>` with one of those out-of-list values, the browser rendered the control as empty/unselected even though a model still existed in config. The config page now normalizes any UI-incompatible base Gemini model value to the first visible dropdown entry before hydrating the form, so migration/default alias values can no longer blank the selector. Version/cache migration now resets the base Gemini model to the first dropdown entry instead of the backend alias, and Quick Setup now seeds the same first entry rather than `gemini-flash-latest`. Also aligned Quick Setup's bundled Gemini advanced defaults to the selected first-entry model so it no longer injects mismatched temperature/context defaults on save.

- **Separated base Gemini model state from advanced Gemini model override state across runtime translation paths:** Server-side normalization previously overwrote `config.geminiModel` with `advancedSettings.geminiModel` whenever advanced settings were enabled, which polluted the saved/base config model, fed unsupported values back into the config-page base dropdown on later reloads, and blurred the distinction between the main model selector and the advanced override selector. Added explicit effective-model resolution so the saved base Gemini model remains the curated config-page value, while runtime translation/provider creation/history/cache metadata still use the advanced Gemini override only when it is explicitly enabled and non-empty. Also updated server-side Gemini availability/validation checks, toolbox/file-upload bootstrap data, and request-scoped Gemini model overrides so they all consistently honor the effective runtime model without re-poisoning the base config UI state.
...
- **Fixed unsupported Stremio ID requests being filtered client-side before SubMaker could log them:** Removed `idPrefixes` from the manifest and added explicit server-side Stremio ID inspection/filtering in the subtitle handler, so unsupported or malformed IDs now reach SubMaker, get logged with the rejection reason, and return an empty subtitle list consistently instead of disappearing with no server visibility.

- **Improved subtitle matching for query-style subtitle requests that already reach SubMaker:** Some Stremio clients were requesting subtitles in a query-string form such as `/subtitles/{type}/{id}.json?filename=...&videoHash=...`, while the Stremio addon SDK only parses subtitle `extra` fields from the path segment form (`/.../filename=...&videoHash=....json`). Added a pre-router normalization step in `index.js` that rewrites query-style subtitle requests into the SDK-compatible path form before the addon router runs, while preserving non-subtitle query params. Also upgraded addon request logging to use the original incoming URL and to explicitly log when subtitle extras were normalized from the query string, making request-shape issues visible in server logs.
...
- **Fixed Sub Toolbox embedded-track language resolution keeping stale guessed languages ahead of trusted xSync/container metadata:** The toolbox extraction page now preserves richer xSync language fields (`languageRaw`, `languageSource`, `languageCode`, `languageIetf`, `languageTag`, etc.), resolves track language with source-aware precedence, and uses that same resolver when saving embedded originals to xEmbed. This prevents weak content/label guesses from surviving after xSync has already recovered a trusted container language, and also improves normalization for exact/regional tags such as `pt-BR`, `pt-PT`, `es-419`, `zh-Hans`, and `zh-Hant`.

- **Fixed xEmbed, xSync, and AUTO entries disappearing after switching streams and coming back:** Local cache-backed subtitle sources were previously rebuilt from a single freshly derived request hash, so revisiting the same stream with missing or drifted filename metadata could make stored xEmbed/xSync/AUTO entries vanish from the subtitle list even though they were still cached. Local subtitle lookups now recover from an ordered set of strong hash candidates built from the current request, remembered per-video stream activity, and persisted strong hash links, so returning to a stream after playing something else no longer drops those local entries as easily.

- **Fixed xEmbed saves/translations not always showing up on the next subtitle refresh:** Saving embedded originals from the toolbox and creating xEmbed translations now bump the per-user subtitle-search revision just like other local subtitle sources, so newly created xEmbed entries invalidate stale cached subtitle lists and appear immediately after the next refresh instead of waiting for unrelated cache churn.

- **Fixed xSync Complete extraction still collapsing on transient stream read slips and overwhelming the live log with FFmpeg noise:** The extension's OPFS-backed full-download path now retries transient read failures up to 3 times with short backoff and resumes from the last written byte via HTTP Range requests instead of restarting from 0 on each network hiccup. Complete-mode recovery also stays strict about rejecting clearly truncated/non-monotonic subtitle outputs rather than surfacing partial fragments as a successful "complete" result, and the offscreen FFmpeg log relay now suppresses repetitive banner/metadata/timestamp-repair spam so real extraction failures are easier to see.

- **Hardened local hash recovery so weak videoId-only fallback hashes do not permanently bridge unrelated releases:** The id-only fallback hash is still available as a last-resort direct lookup seed, but it is no longer used as the primary local subtitle hash or persisted into the shared hash-association graph. This keeps SMDB plus local xEmbed/xSync/AUTO recovery working from stronger stream identity while reducing the risk of cross-release bleed between different files for the same episode or movie.
