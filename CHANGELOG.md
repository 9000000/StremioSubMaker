...
- **VAD filter option for Turbo model:** Added a "Enable VAD filter" checkbox to auto-subs Step 2 (visible only when Cloudflare mode + Turbo model is selected). Voice Activity Detection preprocessing removes silence from audio for cleaner transcription results.
