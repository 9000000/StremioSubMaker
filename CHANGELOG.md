...
- **Improved subtitle variant metadata for current Stremio clients:** Subtitle, `Make`, and `Learn` results now include Stremio's per-variant `label`, using the source filename and provider so variants no longer repeat only the language name. Translation and Learn source matching also recognizes equivalent provider language codes for downloaded and embedded subtitles.

**Bug Fixes:**

- **Kept self-hosted cache budgets safely below the bundled Redis limit:** Ordinary self-hosted non-session defaults now total 2.5 GiB, reserving 1.5 GiB of the Compose deployment's 4 GiB `maxmemory` for sessions, metadata/indexes, allocator fragmentation, and persistence overhead. Every positive `CACHE_LIMIT_*` override still wins, Redis reports a startup warning when explicit budgets consume at least 80% of a configured `maxmemory`, and the Docker guide now consistently keeps `noeviction` so a global LRU policy cannot delete persistent sessions.

- **Made Redis cache quotas and eviction concurrency-safe across replicas:** Cache admission, content/metadata/LRU writes, and size-counter updates now execute in one short server-side atomic operation, while each eviction/delete atomically removes the entry and adjusts its counter, preventing concurrent SubMaker pods from passing the same stale check and overshooting a cache limit. Cleanup no longer evicts healthy caches merely for crossing 80% of their limit, no longer skips every second LRU batch after deletions, and can subtract TTL-expired entry sizes instead of leaving permanently inflated counters that eventually block valid cache writes. Oversized single entries are rejected locally rather than exceeding the entire cache quota, and unlimited session storage still never creates or decrements a byte counter.

**Security Fixes:**

- **Eliminated fabricated OpenSubtitles V3 and Wyzie download-ID SSRF without a CDN allowlist:** Newly returned V3 and Wyzie file IDs now contain AES-GCM-authenticated opaque URL tokens instead of caller-editable base64url destinations, so clients cannot read, fabricate, or retarget the embedded download URL. The bounded route ceiling accommodates the encoder's full 4 KiB dynamic signed-CDN URL limit instead of accepting only today's shorter hosts. Unsigned pre-v1.4.92 IDs are rejected by default; self-hosters can temporarily enable `ALLOW_LEGACY_UNSIGNED_SUBTITLE_URL_IDS=true` for stale client results, but that compatibility path receives the same mandatory network protections and cannot opt into private endpoints.

- **Applied a host-agnostic public-network guard to every untrusted subtitle URL and redirect:** OpenSubtitles V3 downloads and optional filename `HEAD` probes, plus Wyzie direct-source downloads, now accept changing public CDN hosts without a maintained allowlist while rejecting credentials, unsafe protocols, loopback, link-local metadata services, private/CGNAT/reserved IPv4, non-global IPv6, mapped-address bypasses, and hostnames with any non-global DNS answer. DNS is checked again by the socket's actual lookup on every connection, each redirect is revalidated (with HTTPS preserved for Wyzie), wrapped redirect-policy failures stop without retries, environment proxy routing cannot bypass the destination check, and Axios aborts decompressed responses while streaming once they exceed the existing 25 MB archive safety ceiling.

- **Prevented same-origin script injection across server-generated pages:** History, SMDB, file upload, Sub Toolbox, Subtitle Sync, and AutoSubs now pass every inline JSON bootstrap value through one HTML-script-safe serializer. Crafted session, video ID, filename, language, configuration, provider, or localized values can no longer close an inline `<script>` element with `</script>` and execute code that reads another configuration from the browser token vault, cached client configuration, or credential-bearing page state. The serializer preserves the original JavaScript values while escaping `<`, `>`, `&`, U+2028, and U+2029, and generated-page regression coverage now checks all six reported paths for both breakout prevention and valid JavaScript.

- **Stopped Subtitle Sync from exposing an unused Gemini credential:** The Sync bootstrap no longer includes the configured Gemini API key because all translation requests already use the server-side configuration token. AutoSubs keeps its AssemblyAI key only in the existing direct xSync transcription workflow where the extension requires it, with the bootstrap now protected by the centralized script-safe serializer.

- **Closed IPv6 and redirect bypasses in custom-provider SSRF protection:** Custom OpenAI-compatible provider URLs now use canonical IP parsing and reject every non-global-unicast IPv4/IPv6 destination by default, including bracketed IPv6, unspecified/loopback/link-local/ULA/multicast/reserved space, carrier-grade NAT, and hexadecimal or dotted IPv4-mapped IPv6 forms. URL credentials and fragments are rejected; DNS is checked through the operating system resolver during validation and again immediately before each socket connection; and custom-provider redirects remain supported only when every resolved hop passes the same URL, IP-literal, and connection-time checks. Environment proxy routing is disabled for public-mode custom-provider requests so it cannot bypass destination resolution. Self-hosters retain `ALLOW_INTERNAL_CUSTOM_ENDPOINTS=true` specifically for configured local/private LLM endpoints, while untrusted subtitle destinations always remain public-network-only.

- **Bounded archive expansion without reducing the existing download allowance:** Subtitle downloads from every buffered provider now abort after the existing generous 25 MB decompressed-response ceiling instead of allocating an unlimited Axios buffer; the independently refreshed anime-ID dataset has a separate 128 MB ceiling. Archive inputs are rejected before Brotli probing when already oversized, Gzip and Brotli decompression stop at the output ceiling, nested compression is limited to four layers, and ZIP/RAR metadata is checked before subtitle expansion for at most 2,048 files, 64 MB per entry, 256 MB cumulative expanded data, and pathological compression ratios above 1,000:1 once expansion reaches 16 MB. TAR and 7z paths enforce the same available entry/byte accounting, and RAR now asks the extractor for only the subtitle selected by the existing episode/format matcher rather than materializing the complete pack. Ordinary season-pack selection and complex ASS/SSA handling are unchanged, while rejected packs return a clear informational subtitle.

- **Capped buffered and streaming AI provider responses:** Gemini, Anthropic, and every OpenAI-compatible provider (including OpenAI, xAI, DeepSeek, Mistral, OpenRouter, Cloudflare Workers AI, and custom endpoints) now enforce a generous 64 MB decompressed-response ceiling on model-list and translation requests for both JSON and SSE responses. This also bounds the existing streaming recovery accumulators, prevents an attacker-controlled custom provider from amplifying an oversized response through retries, and surfaces a specific non-retryable provider-response-too-large message instead of allowing the Node.js process to buffer indefinitely.
